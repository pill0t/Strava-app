...
with open('strava_tokens.json') as json_file:
    strava_tokens = json.load(
        json_file)  ## If access_token has expired then use the refresh_token to get the new access_token
if strava_tokens['expires_at'] < time.time():  # Make Strava auth API call with current refresh token
    response = requests.post(
        ...
        data={
            ...
            'refresh_token': strava_tokens[os.getenv("refresh_token")] #Here your refresh token
        }
...
while True:
    ...
    page += 1
    activities.to_csv('strava_activities111.csv')